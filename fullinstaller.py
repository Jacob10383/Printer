...
class PrinterInstaller:
    ...
    def backup_moonraker_stats(self) -> None:
    ...
    def restore_moonraker_stats(self) -> None:
        if not self.preserve_stats:
            return
        ...
        self.executor.run(
            f"service {self.config.moonraker_service} stop",
            timeout=30,
        )
        ...
        self.executor.run(
            f"service {self.config.moonraker_service} start",
            timeout=30,
        )

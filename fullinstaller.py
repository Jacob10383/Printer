...
class PrinterInstaller:
    def __init__(
        ...
        # Handle bootstrap path for both frozen (compiled) and normal execution
        if getattr(sys, 'frozen', False):
            ...
            if hasattr(sys, '_MEIPASS'):
                ...
                # Nuitka style - data files are in the same directory as the executable
    ...
    def setup_logging(self) -> None:
        ...
        class _ColorFormatter(logging.Formatter):
            def format(self, record: logging.LogRecord) -> str:
                ...
                elif getattr(record, "to_console", True) and getattr(record, "is_step", False):
                    return f"\033[96m{msg}\033[0m"
                return msg

        # Use UTF-8 encoding with error handling to prevent Windows cp1252 crashes
        console_handler = logging.StreamHandler(sys.stdout)
        console_handler.setLevel(logging.DEBUG)  # Send all messages to GUI for verbose buffer
    ...
    def install_public_key(self) -> bool:
        ...
        pubkey_path = next(
            (ssh_dir / key_name for key_name in possible_keys if (ssh_dir / key_name).exists()),
            None,
        )
    ...
    def ensure_bootstrap_archive(self) -> None:
        ...
        self.log("Local bootstrap archive missing; attempting to download latest release...")
    ...
    def _download_bootstrap_archive(self) -> None:
        ...
        for url in BOOTSTRAP_DOWNLOAD_URLS:
            ...
            try:
                ...
                if tmp_path is None:
                    raise InstallerError("Failed to write bootstrap archive to temporary file")
                ...
                last_error = exc
                self.file_log(f"Failed to download bootstrap archive from {url}: {exc}", "WARNING")
    ...
    def backup_moonraker_stats(
        self, *, force: bool = False, backup_dir: Optional[Path] = None, use_temp: bool = False
    ) -> None:
        ...
        self.moonraker_backup_dir = base_dir / f"moonraker_backup_{self.printer_ip}_{timestamp}"
...
Examples:
  ...
  python3 fullinstaller.py 192.168.1.100 main
    ...
    if modes_selected > 1:
        print("ERROR: Choose only one mode: --key-only, --backup-only, --restore-only, or --reset-only.")
        sys.exit(2)
    ...
    if args.key_only or args.backup_only is not False or args.restore_only:
        if args.reset or args.reset_only or args.preserve_stats or args.restore_backup:
            print("ERROR: --key-only, --backup-only, and --restore-only cannot be combined with --reset, --reset-only, --preserve-stats, or --restore-backup.")
            sys.exit(2)
    ...
    if args.preserve_stats and not (args.reset or args.reset_only):
        print("ERROR: --preserve-stats/--backup can only be used together with --reset or --reset-only.")
        sys.exit(2)
    ...
    selected_steps_provided = any(
        (args.run_bootstrap, args.run_k2, args.run_repo)
    )
    installer.install(

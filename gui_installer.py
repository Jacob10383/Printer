...
class PrinterInstallerGUI:
    ...
    def configure_multiprocessing(self) -> None:
        """Configure multiprocessing to use the embedded interpreter when frozen."""
        try:
            mp.set_start_method("spawn", force=True)
        except RuntimeError:
            pass

        launcher = self.build_launcher_prefix()
        if launcher:
            mp.set_executable(launcher[0])
    ...
    def load_branch_options(self):
        """Fetch branch list from GitHub and populate dropdown."""
    ...
    def build_launcher_prefix(self) -> list[str]:
        """Resolve the Python executable inside frozen bundles."""
        if getattr(sys, "frozen", False):
            exe_path = Path(sys.argv[0]).resolve()
            # macOS app bundle layout: Contents/MacOS/Python
            embedded_python = exe_path.parent / "Python"
            if embedded_python.exists():
                if not os.access(embedded_python, os.X_OK):
                    with contextlib.suppress(Exception):
                        embedded_python.chmod(
                            embedded_python.stat().st_mode
                            | stat.S_IXUSR
                            | stat.S_IXGRP
                            | stat.S_IXOTH
                        )
                if os.access(embedded_python, os.X_OK):
                    return [str(embedded_python)]
            # Windows standalone bundles typically ship python.exe alongside the launcher
            win_python = exe_path.parent / "python.exe"

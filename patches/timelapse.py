...
class Timelapse:
    ...
    def __init__(self, confighelper: ConfigHelper) -> None:
        ...
        self.config: Dict[str, Any] = {
            ...
            "previewimage": True,
            "saveframes": False,
            "transcoder_endpoint": "http://192.168.50.11:8765/timelapse-complete",
        }
        ...
        if not self.ffmpeg_installed:
            ...
            logging.info(
                f"timelapse: {self.ffmpeg_binary_path} \
    ...
    async def getWebcamConfig(self) -> None:
        ...
        try:
            ...
            if not cams:
                logging.info(
                    "WARNING: no camera configured, " + "using the fallback config"
                )
                fallback = {
    ...
    async def render(self, webrequest=None):
        ...
        if not filelist:
            ...
            if cmdstatus:
                ...
                # Send notification to transcoder service
                if self.config.get("transcoder_endpoint"):
                    try:
                        payload = json.dumps(
                            {
                                "filename": f"{outfile}.mp4",
                                "path": self.out_dir + outfile + ".mp4",
                            }
                        ).encode("utf-8")

                        req = urllib.request.Request(
                            self.config["transcoder_endpoint"],
                            data=payload,
                            headers={"Content-Type": "application/json"},
                        )
                        urllib.request.urlopen(req, timeout=5)
                        logging.info(f"Sent transcoder notification for {outfile}.mp4")
                    except Exception as e:
                        logging.warning(f"Failed to send transcoder notification: {e}")
